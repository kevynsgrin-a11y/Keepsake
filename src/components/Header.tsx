...
import { BookOpen, Calendar, Scroll, Users, Clock, Printer, Plus, Search, Sparkles, X } from 'lucide-react';
import { buildTodayAlmanac } from '../utils/almanac';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const almanac = useMemo(() => buildTodayAlmanac(new Date()), []);

  const navItems = [
    { path: '/almanac', label: 'Daily Almanac', icon: BookOpen },
    { path: '/vault', label: 'Milestone Vault', icon: Scroll },
    { path: '/calendar', label: 'Heirloom Calendar', icon: Calendar },
    { path: '/heritage', label: 'Family Heritage', icon: Users },
    { path: '/capsules', label: 'Time Capsules', icon: Clock },
    { path: '/export', label: 'Print & Export', icon: Printer },
  ];

  return (
    <header className="sticky top-0 z-40 bg-amber-50/90 backdrop-blur-md border-b border-amber-200/60 shadow-sm no-print">
      {/* Top Banner Bar — hidden on small screens so it doesn't push primary content below the fold */}
      <div className="hidden sm:block bg-gradient-to-r from-amber-950 via-stone-900 to-amber-950 text-amber-100/90 text-xs py-1.5 px-4">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          {/* Logo & Title */}
          <Link to="/almanac" className="flex items-center space-x-3.5">
            ...
            <div>
              ...
              <p className="text-xs text-amber-900 font-garamond italic mt-0.5">
          ...
          <div className="flex items-center space-x-3">
            <div className="relative flex-1 md:w-64">
              <label htmlFor="site-search" className="sr-only">Search stories, recipes, dates</label>
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 text-sm bg-white/90 border border-amber-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-600 focus:border-amber-500 text-stone-800 placeholder-stone-400 transition"
              />
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery('')}
                  aria-label="Clear search"
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-700 p-1 rounded focus:outline-none focus:ring-2 focus:ring-amber-600"
                >
        ...
        <nav
          ...
          className="flex space-x-1 sm:space-x-2 overflow-x-auto pt-4 mt-2 border-t border-amber-200/50 scrollbar-none [mask-image:linear-gradient(to_right,black_92%,transparent)] sm:[mask-image:none]"
        >
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`w-4 h-4 ${isActive ? 'text-amber-300' : 'text-amber-700/70'}`} aria-hidden="true" />
